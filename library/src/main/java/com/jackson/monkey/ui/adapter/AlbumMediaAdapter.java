 ...
 */
package com.jackson.monkey.ui.adapter;
...
import com.jackson.monkey.entity.IncapableCause;
...
import com.jackson.monkey.ui.widget.CheckView;
import com.jackson.monkey.ui.widget.MediaGrid;
...
public class AlbumMediaAdapter extends
    ...
    private int getDateCountFromCursor(Cursor mCursor) {
        ...
        while (mCursor.moveToNext()) {
            ...
            if (!TextUtils.isEmpty(date)) {
                ...
                //为每一个cursor的位置，映射一个日期
                cursorToDateMap.put(mCursor.getPosition(), mDateList.size() - 1);

                //如果这个cursor的位置与上一个cursor所映射的日期不一致，则视图先添加一个日期视图
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (!mSelectionSpec.groupByDate) {
            ...
            //获取此位置对应的视图，日期视图或者cursor视图
            Integer cursorPos = viewToCursorMap.get(position);
            //如果cursorPos不为空，则说明是日期视图，移动至此位置。
            if (cursorPos != null) {
    ...
    protected void onBindViewHolder(final RecyclerView.ViewHolder holder, Cursor cursor) {
        if (holder instanceof CaptureViewHolder) {
            ...
            Log.i(TAG, "onBindViewHolder: MediaItem "+item.getUri().toString());

            //如果包含了此Uri,并且这个Item还未被选中
            ...
            Integer integer = viewToCursorMap.get(holder.getAdapterPosition() + 1);
            int nextCursorPos = -1;
            if (integer != null) {
                nextCursorPos = integer;
            }
            if (nextCursorPos == -1) {
                mediaDateViewHolder.mDate.setText("");
            }else {
                mediaDateViewHolder.mDate.setText(mDateList.get(cursorToDateMap.get(nextCursorPos)));
            }
        }
    }
    ...
    public int getItemViewType(int position) {
        ...
        //enable groupByDate
        Integer cursorPosition = viewToCursorMap.get(position);
